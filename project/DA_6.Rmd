...
d <- insur[,1:3]
p1 <- pairs(d, color="lightseagreen")
p2 <- boxplot(data = insur, charges ~ smoker)
#grid.arrange(p1, p2, ncol = 2)

require(GGally)
ggpairs(insur, mapping = ggplot2::aes(colour = smoker))
```
...
ggplot(regression.points1, aes(x = charges_hat, y = residual)) + 
  ...
  facet_wrap(~ smoker)
```
The points in Figure \ref {fig:resid2} have zero mean and constant variance, but there are a number of residuals with large variance above the zero line.  The linear model assumptions are somewhat dubious, and further investigation could illuminate possible underlying covariate patterns.  
